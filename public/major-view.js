...
export function renderMajors(target,universityId='') {
  ...
  let version=0, query='';
  async function search(offset=0,moveFocus=false) {
    ...
    try {
      ...
      if(universityId&&!data){output.innerHTML='<section class="empty"><h2>이 대학의 학과 목록을 아직 확보하지 못했어요</h2><p>학과가 없다는 뜻은 아닙니다. 대학 안내에서 공식 자료를 확인해 주세요.</p><a class="button" href="#university/'+esc(universityId)+'">대학 안내로 돌아가기</a><a class="button" href="#majors">전국 학과 검색</a></section>';return;}
      ...
      if(document.getElementById('major-next'))document.getElementById('major-next').onclick=()=>search(data.nextOffset,true);
      if(document.getElementById('major-prev'))document.getElementById('major-prev').onclick=()=>search(Math.max(0,offset-50),true);
      if(moveFocus){output.tabIndex=-1;output.focus();output.scrollIntoView({block:'start'});}
    } catch {
      if(request!==version||!output.isConnected)return;
  ...
  if(universityId){form.hidden=true;form.insertAdjacentHTML('beforebegin','<div class="actions"><a class="button" href="#university/'+esc(universityId)+'">← 대학 안내로 돌아가기</a><a class="button" href="#history/'+esc(universityId)+'">이 대학 입결 비교</a><a class="button" href="#majors">전국 학과 검색</a></div>');}
