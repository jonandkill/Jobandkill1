...
export function renderRecommendations(target, {profile = {}, universities = [], outcomeSchools = [], onSave, onProfileChange} = {}) {
  ...
  if (parseGrade(local.average, local.scale) !== null) requestQuickRanking();
  ...
  function showPendingStatus(){
    ...
    $('candidate-ranking-results').innerHTML='';
  }
  ...
  function drawSchools(moveFocus=false) {
    ...
    ownedOutput.innerHTML=`<div class="section-head school-result-summary"><div><h2 tabindex="-1">학교 ${candidates.length}곳을 찾았어요</h2><p>${esc(local.preferredRegion||'전국')} · ${esc(local.preferredType||'전체 대학')}${parseGrade(local.average,local.scale)!==null?' · 내 평균 '+esc(local.average)+'등급':''}</p></div></div><p class="hint">이 목록은 학교를 찾아보는 가나다순 탐색 목록입니다. 성적 기준 1·2·3순위는 위 후보 카드에서만 표시하며, 학과·전형별 공식 입결을 근거로 합니다.</p>${candidates.length?`<div class="cards">${paged.items.map(u=>`<article class="card school-candidate"><span class="tag">${u.outcomeSummary?.years?.length||0}개년 입결 ${u.outcomeSummary?.years?.length?'제공':'수집 중'}</span><h3>${esc(u.displayName||u.name)}</h3><p>${esc(u.campus||'본교')} · ${esc(u.region)} · ${esc(u.institutionType)}</p><dl class="facts"><div><dt>수집 입결</dt><dd>${u.outcomeSummary?.count||0}건</dd></div><div><dt>자료 연도</dt><dd>${u.outcomeSummary?.years?.length?esc(u.outcomeSummary.years.join(' · ')):'아직 미수집'}</dd></div></dl><div class="actions"><button class="primary" type="button" data-open-school="${esc(u.id)}">학과·입결 비교</button><a class="button" href="#university/${esc(u.id)}">대학 상세</a>${onSave?`<button type="button" data-save-school="${esc(u.id)}" aria-label="${esc(u.name)} 후보 담기">+ 후보</button>`:''}</div></article>`).join('')}</div>${pageControls(paged.page,paged.totalPages,'recommend-schools','학교 목록')}`:'<div class="empty"><h3>검색 조건에 맞는 학교가 없어요</h3><p>지원 가능한 학교가 없다는 뜻은 아닙니다. 검색어를 줄이거나 위 ‘전국 학교 보기’를 누르세요.</p></div>'}`;
